...
// 이벤트의 주체 마우스 & 손가락
// click : 마우스를 클릭했다가 떼는 순간 이벤트가 발생!
...
const setTranslateX = (x) => {
  ...
  return parseInt(
    (hashContent.style.transform = `translateX(${x}px`).transform.split(/[^\-0-9]+/g)[5]);
...
const onScrollEnd = (e) => {
  ...
  if(listX>0) {
    setTranslateX(0);
    hashContent.style.transition = "all 0.1s else";
    listX = 0;
  } else id ( listX < listClientWidth-listScrollWidth;) {
