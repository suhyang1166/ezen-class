...
// 데이터를 체계적으로 보관,관리,사용가능
// 단점 : for...in => 객체안에 있는 value값만을 찾아와서 직접적으로 반복문을 돌릴 수 있는 방법 X
// 단점 : 객체안에 구성되어있는 아이템들은 배열처럼 인덱스 번호를 할당받지 못한다!

// 3) 배열객체
// 장점 : 배열안에 포함되어있는 아이템들이 고유의 인덱스 값을 가지고 있으며, for문을 활용한 반복문으로 업무의 효율성 극대화
// 단점 : 객체처럼 key & value 형태로 구성된 체계적인 데이터 관리 X

// Solution : 일반객체 & 배열객체의 장점만 모아서 하나로 만들어 주면 어떨까? => Map 객체

// Map 객체 > key, value 값으로 구성된 아이템을 가질 수 있다
// > 각 아이템별 인덱스 값을 할당
// for의 반복문을 활용해서 언제든지 값을 추출

// const bag = new Map();
// bag.set("color", "red"); // set() : key & value 값 생성 ("key", "value")
