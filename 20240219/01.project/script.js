...
// 중급 레벨 단계
// 한계 : 만약 숫자를 1~5가 아니라 1~100을 더해야한다면 재활용 불가!
// let sum = 0;
// for (let i = 1; i <= 5; i++) {
//   sum += i;
// }
...
// 고급 레벨 단계
// 재활용이 가능한 코드 업그레이드
// function calcSum=함수명(함수의 매개변수공간)
// function calcSum(n) {
//   let sum = 0;
//   for (let i = 1; i <= n; i++) {
//     sum += i;
//   }
//   console.log(`1부터 ${n}까지 더해면 ${sum}입니다.`);
// }

// calcSum(); >> 함수를 호출했다/구현했다
// calcSum(40);
...
// 2)가급적이면, 전역 스코프의 변수 선언은 피해라! >> 언제라도 다른 코드에 영향을 미치기에 너무 남발하지X
...
// 나중에 고차함수를 사용할때 자주 사용 예정 >> 현재는 많이 사용 X

// Hoisting 기법
// 무언가를 끌어올리다!!
// 전통적인 class함수는 hoisting이 가능 / 화살표 & 익명함수는 불가능
// example();
...
// 2)JS 근본없는 구현 & 호출부에 순서를 명확하게 정의하기 위한 목적!
...
// // btn.addEventListener("이벤트의 종류", 이벤트의 종류에서의 실행 함수=콜백함수가 들어와야함)
// // const display = () => {
// //   alert("오늘도 화이팅 입니다!");
// // };
...
// 모든 프로그레밍 언어에서 1급시민이 되려면 다음과 같은 조건에 부합 해야함!

// 1급 시민이 되려면 조건이 있음
// 1) 함수를 변수에 값으로 할당할 수 있어야 한다!
// 2) 함수가 또다른 함수의 매개변수 혹은 인자값으로 사용될 수 있어야한다! >> 콜백함수로 쓸수있어야한다
// 3) 함수가 또다른 함수의 return문 안에 반환값으로 들어올 수 있어야한다! => 고차함수
...
// const addNum = (...>>전개연산자>>배열형태를 띔!numbers) => {
...
// setInterval() : 일정시간마다 반복해서 무언가를 처리하는 함수!
...
// clearInterval() : 특정조건에 따라서 반복실행 함수를 멈추게 하는 함수
// 단락회로평가! >> 기준값을 정해두고
